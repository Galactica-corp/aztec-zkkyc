...
const styles = {
  ...
  kycSection:
    'space-y-3 rounded-lg border border-default bg-surface-tertiary p-3',
...
export const CertificateRegistryCard: React.FC = () => {
  ...
  const [kycBirthdayDate, setKycBirthdayDate] = useState(
    DEFAULT_KYC_BIRTHDAY_DATE
  );
  const [kycCitizenship, setKycCitizenship] = useState('DEU');
  const [kycVerificationLevel, setKycVerificationLevel] = useState(
    DEFAULT_KYC_VERIFICATION_LEVEL
  );
  const [kycStreetAndNumber, setKycStreetAndNumber] =
    useState('MUSTERSTRASSE 10');
  ...
  const [checkWithRequirementsStatus, setCheckWithRequirementsStatus] =
    useState<'idle' | 'pending' | 'success'>('idle');
  ...
  const handleIssueCertificate = useCallback(async () => {
    ...
    const missingValue = requiredTextValues.find(
      ([, value]) => value.trim() === ''
    );
    if (missingValue) {
  ...
  const handleRevokeCertificate = useCallback(async () => {
    ...
    if (revocationId === null) {
      toastError(
        'Invalid field',
        'revocation_id must be a non-negative integer'
      );
      return;
    }
    try {
              ...
              <TabsContent value="guardian" className={styles.tabContent}>
                <section className={styles.section}>
                  ...
                  <section className={styles.kycSection}>
                    <h4 className={styles.kycSectionTitle}>
                      KYC personal note
                    </h4>
                    <div className={styles.kycGrid}>
                      <div className={styles.formGroup}>
                        <label
                          htmlFor="cert-kyc-surname"
                          className={styles.label}
                        >
                          surname
                        </label>
                      ...
                      <div className={styles.formGroup}>
                        <label
                          htmlFor="cert-kyc-birthday-unix"
                          className={styles.label}
                        >
                          birthday (UTC date)
                        </label>
                      ...
                      <div className={styles.formGroup}>
                        <label
                          htmlFor="cert-kyc-verification-level"
                          className={styles.label}
                        >
                  ...
                  <section className={styles.kycSection}>
                    ...
                    <div className={styles.kycGrid}>
                      <div className={styles.formGroup}>
                        <label
                          htmlFor="cert-kyc-street-and-number"
                          className={styles.label}
                        >
                          street_and_number
                        </label>
                        <Input
                          ...
                          onChange={(e) =>
                            setKycStreetAndNumber(e.target.value)
                          }
              ...
              <TabsContent value="user" className={styles.tabContent}>
                ...
                <section className={styles.section}>
                  <h3 className={styles.sectionTitle}>
                    Check certificate with requirements
                  </h3>
                  <p className={styles.checkHelper}>
                    Uses your connected account, authwit_nonce 0, and a
                    requirement checker contract.
                  </p>
                  <div className={styles.formGroup}>
                    <label
                      htmlFor="cert-check-requirement-address"
                      className={styles.label}
                    >
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setCheckRequirementAddress(e.target.value)
                      }
                      placeholder="0x..."
                      disabled={isProcessing || !contractsReady}
                    />
